...
class SimpleTestCase(ContractModules390TestCase):
    filepath = os.environ.get("DATA_SIMPLE_V390", "contracts_sdk/example_unit_tests/contract_modules/simple_v390.py")
    ...
    def test_build_posting_instruction_batch_fails_with_negative_amount(self):
        ...
        with self.assertRaises(types.Rejected) as ex:
            self.run_contract_function(self.contract_code, "build_posting_instruction_batch", amount, self.effective_date)
    ...
    def test_build_posting_instruction_batch_with_positive_amount(self):
        amount = Decimal("5")
        posting_instruction_batch = self.run_contract_function(self.contract_code, "build_posting_instruction_batch", amount, self.effective_date)
