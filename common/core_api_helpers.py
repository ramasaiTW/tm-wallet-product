...
from common.api_helpers import (paginated_request, send_request)

from inception_sdk.test_framework.contracts.simulation.vault_caller import response_logger
...
def get_all_customers(status=None, page_size=200, all_pages=True):
    ...
    return paginated_request('get', '/v1/customers', parameters, page_size, all_pages)


def get_account_info(account_id, parameters=None):
    return send_request('get', f'/v1/accounts/{account_id}', parameters)


def modify_product_parameter(product_version_id: str, items_to_add=None, items_to_remove=None, request_id='') -> dict[
    str, str]:
    ...
    items_data = {"request_id": request_id, "items_to_add": items_to_add, "items_to_remove": items_to_remove}
...
def get_param_timeseries_by_product_version(product_version_id) -> List[Any]:
    return send_request('get', f'/v1/product-versions/{product_version_id}:paramTimeseries')


def get_account_param_timeseries(account_id) -> List[Any]:
    return send_request('get', f'/v1/accounts/{account_id}:paramTimeseries')


def get_all_products(include_internality='INTERNALITY_REFINER_EXTERNAL', page_size=10, all_pages=True):
    parameters = {
        "include_internality": include_internality,
    }

    return paginated_request('get', '/v1/products', parameters, page_size, all_pages)
...
def get_multiple_product_versions(product_version_id, include_parameters=False) -> List[Any]:
    ...
    return send_request('get', '/v1/product-versions:batchGet', parameters)


def get_all_product_versions(product_id, include_parameters=False, page_size=10, all_pages=True) -> List[Any]:
    parameters = {
    ...
    return paginated_request('get', '/v1/product-versions', parameters, page_size, all_pages)


def get_all_accounts(account_statuses=None, product_version_ids=None, opening_timestamp_from=None,
                  opening_timestamp_to=None, page_size=100, all_pages=True) -> List[Any]:
    ...
    return paginated_request('get', '/v1/accounts', parameters, page_size, all_pages)
...
def get_live_balances(account_id, account_address=None, page_size=100, all_pages=True) -> List[Any]:
    ...
    return paginated_request('get', '/v1/balances/live', parameters, page_size, all_pages)
...
def get_account_updates(account_id: str, page_size=100, all_pages=True) -> List[Any]:
    ...
    return paginated_request('get', '/v1/account-updates', parameters, page_size, all_pages)


def migrate_account_data(account_id,
                    product_version_id,
    ...
    response = send_request(
        'post', '/v1/account-updates', data=json.dumps(migration_payload)
    )
...
def migrate_account_between_products(
    ...
    response = send_request(
        'post', '/v1/account-migrations', data=json.dumps(migration_payload)
    )
...
def get_account_migrations_by_id(account_migration_id: str):
    parameters = {
        "ids": account_migration_id
    }
    response = send_request(
        'get', '/v1/account-migrations:batchGet', parameters,
    )
...
def activate_account(account_id, request_id=''):
    ...
    response = send_request(
        'post', '/v1/account-updates', data=json.dumps(update_payload)
    )
    ...
    update_payload = {
        ...
        "account_update": {
            ...
            "instance_param_vals_update": {
                "instance_param_vals": instance_jdata
            }
        }
    }
    response = send_request(
...
def get_all_schedules(status='SCHEDULE_STATUS_FAILED', page_size=100, all_pages=True, name_filter=None):
    parameters = {}
    if status:
    ...
    return paginated_request('get', '/v1/schedules', parameters, page_size, all_pages)
...
def get_account_schedule_mappings(account_id, page_size=100, all_pages=True):
    ...
    return paginated_request('get', '/v1/account-schedule-assocs', parameters, page_size, all_pages)
...
def get_schedules_by_ids(schedule_ids):
    ...
    return send_request('get', '/v1/schedules:batchGet?' + schedule_id_param)
...
def get_all_jobs(sched_id, page_size=10, all_pages=True):
    ...
    return paginated_request('get', '/v1/jobs', parameters, page_size, all_pages)
...
def get_posting_instruction_batches(start_time, end_time, page_size=50, all_pages=True):
    ...
    return paginated_request('get', '/v1/posting-instruction-batches', parameters, page_size, all_pages)


def get_posting_instruction_batch_by_client_id(client_batch_id, page_size=50, all_pages=True):
    parameters = {
        "client_batch_ids": client_batch_id
    }

    return paginated_request('get', '/v1/posting-instruction-batches', parameters, page_size, all_pages)


def get_posting_instruction_batch_by_account_id(account_ids, posting_start_time, posting_end_time, page_size=100,
                                                 all_pages=True):
    parameters = {
...
def get_post_posting_failure_logs(page_size=50, all_pages=True):
    ...
    return paginated_request('get', '/v1/post-posting-failures', parameters, page_size, all_pages)


def republish_failed__post_postings(account_id, republish_type="REPUBLISH_TYPE_REPUBLISH_SINGLE_FAILURE"):
    data = {
        "account_id": account_id,
        "republish_type": republish_type
    }

    return send_request(method='post', path='/v1/post-posting-failures:republish', data=json.dumps(data))


def republish_existing_job(job_id):
    response = send_request('post', f'/v1/jobs/{job_id}:republish')
...
def list_service_accounts_by_status(status, page_size=50, all_pages=True):
    parameters = {
        "service_account_statuses": status
    }

    return paginated_request('get', '/v1/service-accounts', parameters, page_size, all_pages)
...
def create_service_account(sa_data, request_id=''):
    ...
    service_account_payload = {
        ...
        "service_account": {
            "name": sa_data["name"],
            "permissions": sa_data["permissions_to_add"]
        }
    }
    response = send_request(
...
def update_service_account_permissions(update_sa_data, request_id=''):
    ...
    response = send_request(
        'put', f'/v1/service-accounts/{update_sa_data["id"]}:updatePermissions', data=json.dumps(service_account_payload)
    )
    return response
...
def update_account_details(account_id, parameters=None):
    return send_request('put', f'/v1/accounts/{account_id}', data=json.dumps(parameters))
...
def get_linked_payment_devices(account_id):
    ...
    return paginated_request('get', '/v1/payment-device-links', parameters)


def disable_payment_device_link(payment_device_link_id):
    update_payload = {
        "payment_device_link": {
            "status": "PAYMENT_DEVICE_LINK_STATUS_INACTIVE"
        },
