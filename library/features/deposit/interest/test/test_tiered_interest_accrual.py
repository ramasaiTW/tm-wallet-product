...
ACCRUED_INTEREST_PAYABLE_ACCOUNT = tiered_interest_accrual.interest_accrual_common.PARAM_ACCRUED_INTEREST_PAYABLE_ACCOUNT
ACCRUED_INTEREST_RECEIVABLE_ACCOUNT = tiered_interest_accrual.interest_accrual_common.PARAM_ACCRUED_INTEREST_RECEIVABLE_ACCOUNT  # noqa: E501
...
class TestInterestAccrual(FeatureTest):
    ...
    def test_accrue_interest_with_positive_accrual(
        ...
        self.assertListEqual(
            tiered_interest_accrual.accrue_interest(vault=mock_vault, effective_datetime=DEFAULT_DATETIME),
...
class TestTieredAccrualAmount(FeatureTest):
    ...
    def test_accrue_interest_with_multiple_tiers(self, mock_determine_tier_balance: MagicMock, mock_yearly_to_daily_rate: MagicMock):
        ...
        self.assertEqual(
            description,
            "Accrual on 4.00 at annual rate of 1.00%. Accrual on 6.00 at annual rate of 2.00%. " "Accrual on 90.00 at annual rate of 3.00%. ",
        )
        ...
        mock_determine_tier_balance.assert_has_calls(
            [
                ...
                call(effective_balance=Decimal(100), tier_min=Decimal("10"), tier_max=Decimal("1000")),
...
class DetermineTierBalance(FeatureTest):
    def test_normal_negative_tiers(self):
        tier_balance = tiered_interest_accrual.determine_tier_balance(effective_balance=Decimal("-5"), tier_max=Decimal("-4"), tier_min=Decimal("-10"))
        self.assertEqual(tier_balance, Decimal("-1"))

    def test_normal_negative_tiers_with_balance_at_max(self):
        tier_balance = tiered_interest_accrual.determine_tier_balance(effective_balance=Decimal("-4"), tier_max=Decimal("-4"), tier_min=Decimal("-10"))
        self.assertEqual(tier_balance, Decimal("0"))

    def test_normal_negative_tiers_with_balance_at_min(self):
        tier_balance = tiered_interest_accrual.determine_tier_balance(effective_balance=Decimal("-10"), tier_max=Decimal("-4"), tier_min=Decimal("-10"))
        self.assertEqual(tier_balance, Decimal("-6"))

    def test_normal_positive_tier(self):
        tier_balance = tiered_interest_accrual.determine_tier_balance(effective_balance=Decimal("5"), tier_max=Decimal("10"), tier_min=Decimal("4"))
        self.assertEqual(tier_balance, Decimal("1"))

    def test_normal_positive_tier_with_balance_at_min(self):
        tier_balance = tiered_interest_accrual.determine_tier_balance(effective_balance=Decimal("4"), tier_max=Decimal("10"), tier_min=Decimal("4"))
        self.assertEqual(tier_balance, Decimal("0"))

    def test_normal_positive_tier_with_balance_at_max(self):
        tier_balance = tiered_interest_accrual.determine_tier_balance(effective_balance=Decimal("10"), tier_max=Decimal("10"), tier_min=Decimal("4"))
        self.assertEqual(tier_balance, Decimal("6"))

    def test_no_tiers_yields_zero(self):
        tier_balance = tiered_interest_accrual.determine_tier_balance(effective_balance=Decimal("10"))
        self.assertEqual(tier_balance, Decimal("0"))

    def test_different_sign_min_max_tiers_yields_zero(self):
        tier_balance = tiered_interest_accrual.determine_tier_balance(effective_balance=Decimal("10"), tier_min=Decimal("-1"), tier_max=Decimal("1"))
        self.assertEqual(tier_balance, Decimal("0"))
    ...
    def test_no_tier_min_and_negative_tier_max(self):
        tier_balance = tiered_interest_accrual.determine_tier_balance(effective_balance=Decimal("-15"), tier_max=Decimal("-10"))
        self.assertEqual(tier_balance, Decimal("-5"))

    def test_no_tier_min_and_positive_tier_max(self):
        tier_balance = tiered_interest_accrual.determine_tier_balance(effective_balance=Decimal("4"), tier_max=Decimal("10"))
        self.assertEqual(tier_balance, Decimal("4"))

    def test_no_tier_max_and_negative_tier_min(self):
        tier_balance = tiered_interest_accrual.determine_tier_balance(effective_balance=Decimal("-15"), tier_min=Decimal("-10"))
