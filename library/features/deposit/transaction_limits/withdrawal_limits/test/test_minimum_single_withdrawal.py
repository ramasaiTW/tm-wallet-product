...
class TestMinimumSingleWithdrawal(CommonTransactionLimitTest):
    ...
    def test_minimum_single_withdrawal_ignore_deposit(self, mock_get_parameter: MagicMock):
        ...
        mock_get_parameter.side_effect = mock_utils_get_parameter({"minimum_withdrawal": Decimal("0.01")})
        self.assertIsNone(minimum_single_withdrawal.validate(vault=MagicMock(), postings=posting_instructions, denomination=DEFAULT_DENOMINATION))
    ...
    def test_minimum_single_withdrawal_not_met(self, mock_get_parameter: MagicMock):
        ...
        expected_rejection = Rejection(
            message="Transaction amount 0.001 GBP is less than the minimum withdrawal " "amount 0.01 GBP.",
            reason_code=RejectionReason.AGAINST_TNC,
        )

        result = minimum_single_withdrawal.validate(vault=MagicMock(), postings=posting_instructions, denomination=DEFAULT_DENOMINATION)
    ...
    @patch.object(minimum_single_withdrawal.utils, "get_parameter")
    def test_minimum_single_withdrawal_several_postings_one_doesnt_meet_limit(self, mock_get_parameter: MagicMock):
        ...
        result = minimum_single_withdrawal.validate(vault=MagicMock(), postings=posting_instructions, denomination=DEFAULT_DENOMINATION)
        expected_rejection = Rejection(
            message="Transaction amount 0.00999 GBP is less than the minimum withdrawal " "amount 0.01 GBP.",
            reason_code=RejectionReason.AGAINST_TNC,
        )
        self.assertEqual(result, expected_rejection)
