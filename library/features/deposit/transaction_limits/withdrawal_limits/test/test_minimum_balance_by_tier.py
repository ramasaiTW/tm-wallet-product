...
class TestMinimumBalanceByTier(CommonTransactionLimitTest):
    ...
    @patch.object(minimum_balance_by_tier.account_tiers, "get_tiered_parameter_value_based_on_account_tier")
    def test_account_tier_minimum_balance_exceeded(
        ...
        mock_get_parameter.side_effect = mock_utils_get_parameter({"tiered_minimum_balance_threshold": {"STANDARD": "0", "SAVINGS": "100"}})
        ...
        expected_rejection = Rejection(
            message="Transaction amount -1 GBP will result in the account balance falling " "below the minimum permitted of 100 GBP.",
    ...
    def test_account_tier_minimum_balance_exceeded_below_zero(
        ...
        expected_rejection = Rejection(
            message="Transaction amount -101 GBP will result in the account balance " "falling below the minimum permitted of 0 GBP.",
            reason_code=RejectionReason.AGAINST_TNC,
        )
    ...
    def test_account_tier_minimum_balance_exceeded_when_balances_argument_is_not_provided(
        ...
        mock_get_account_tier.side_effect = "STANDARD"
        mock_get_tiered_parameter_value_based_on_account_tier.side_effect = [Decimal("0")]
        mock_vault = self.create_mock(balances_observation_fetchers_mapping=test_balance_observation_fetcher_mapping)
        result = minimum_balance_by_tier.validate(
            vault=mock_vault,
            postings=posting_instructions,
            denomination=DEFAULT_DENOMINATION,
    ...
    def test_account_tier_minimum_balance_multi_postings_limit_exceeded(
        ...
        expected_rejection = Rejection(
            message="Transaction amount -100.01 GBP will result in the account balance " "falling below the minimum permitted of 0 GBP.",
            reason_code=RejectionReason.AGAINST_TNC,
        )
