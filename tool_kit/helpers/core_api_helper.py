...
def api_request(http_method, endpoint, query_params=None, data=None):
    url = construct_full_url(endpoint)
    if "X-Auth-Token" not in session.headers:
        setup_http_client()

    try:
        r = session.request(http_method, url, params=query_params, data=data)
        r.raise_for_status()
...
def load_paginated_data(
    ...
    while True:
        ...
        resp = api_request(http_method, endpoint, query_params)
